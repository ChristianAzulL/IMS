...
if ($from && $to) {
    ...
    if ($supplier_res->num_rows > 0) {
        while ($row = $supplier_res->fetch_assoc()) {
            $net = $row['sup_gross_sale'] - $row['sup_unit_cost'];
            $csv_rows[] = [
                $num, $row['supplier'], '', '', '', '', '', '', '', '', '', '',
                $row['sup_outbounded_qty'], // quantity here
                number_format($row['sup_unit_cost'], 2),
                number_format($row['sup_gross_sale'], 2),
                number_format($net, 2)
            ];
            ...
            if ($cat_res->num_rows > 0) {
                while ($cat = $cat_res->fetch_assoc()) {
                    $net = $cat['gross_sale'] - $cat['unit_cost'];
...
// --- Generate heading section with 15 columns ---
$heading_rows = [
...
// Process categories and suppliers for heading
...
fputcsv($output, array_fill(0, 16, ''));

// Table header (same 15 columns)
fputcsv($output, [
    '#', 'SUPPLIER', 'ORDER #', 'OUTBOUND #', 'CUSTOMER',
...
foreach ($csv_rows as $row) {
    fputcsv($output, array_pad($row, 16, ''));
}
