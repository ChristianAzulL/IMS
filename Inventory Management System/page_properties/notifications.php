...
if ($result1->num_rows > 0) {
    while ($row1 = $result1->fetch_assoc()) { // Corrected from $result to $result1
        ...
        $notification2 = "SELECT
                            ...
                            `date`
                          FROM stocks
                          WHERE warehouse IN ($user_warehouse_id)
                          AND product_id = '$not_product_id'";
        ...
        if ($result2->num_rows > 0) {
            while ($row2 = $result2->fetch_assoc()) { // Corrected from $result to $result2
                ...
                if ($not_totalMonths >= 1 && $not_totalMonths < 2) {
                    if ($not_item_status == 0 || $not_item_status == 2 || $not_item_status == 3) {
                        echo "<br>Description: $not_product_description<br>Brand: $not_brand_name<br>Category: $not_category_name<br>is now more than 1 month on our store since $not_date_added";
                    }
                } elseif ($not_totalMonths >= 2 && $not_totalMonths < 3) {
                    if ($not_item_status == 0 || $not_item_status == 2 || $not_item_status == 3) {
                        echo "<br>Description: $not_product_description<br>Brand: $not_brand_name<br>Category: $not_category_name<br>is now more than 2 months on our store since $not_date_added";
                    }
                } elseif ($not_totalMonths >= 3) {
                    if ($not_item_status == 0 || $not_item_status == 2 || $not_item_status == 3) {
                        echo "<br>Description: $not_product_description<br>Brand: $not_brand_name<br>Category: $not_category_name<br>is now more than 3 months on our store since $not_date_added";
                    }
                }
            }
        }
        ...
        if ($notification_item_quantity <= $not_safety) {
            echo "<br>---------------------------<br>Description: $not_product_description<br>Brand: $not_brand_name<br>Category: $not_category_name<br>Quantity: $notification_item_quantity<br>Note: Quantity is less than the safety level.<br>-------------------------------";
        }
    }
}
?>
